...
- **Phase 3 — Interrupt controller + keypad.** `hw/intc/gba_intc.c` (flat IE/IF/IME merge, modeled on `hw/intc/imx_avic.c`). `hw/input/gba_keypad.c` (KEYINPUT/KEYCNT). **Done.** `include/hw/intc/gba_intc.h` defines the `GBA_IRQ_*` enum (0=VBlank..13=GamePak) that every other GBA device's IRQ output connects to via `qdev_get_gpio_in(intc, GBA_IRQ_x)` — this is the shared wiring contract for all later phases (timers, DMA, PPU, SIO). Keypad uses a fixed (non-configurable) keyboard mapping via `qemu_input_handler_register`/`qemu_input_linux_to_qcode` (pattern: `hw/input/stellaris_gamepad.c`) since the GBA's 10 buttons are fixed hardware, unlike a generic gamepad: arrow keys = D-pad, X/Z = A/B, Return/Backspace = Start/Select, A/S = L/R.
- **Phase 4 — Timers.** `hw/timer/gba_timer.c`, 4x `ptimer`-based counters (pattern: `hw/timer/cmsdk-apb-timer.c`), manual cascade linking between instances. **Done.** One `GBATimerState` device models all 4 units (avoids cross-device QOM links for cascade chaining -- it's just array indexing internally). Since `ptimer` is a down-counter but GBA timers are up-counters, each non-cascade unit's "ticks remaining until overflow" = `0x10000 - reload`, and the live up-counter value is recovered on read as `0x10000 - ptimer_get_count()`. Cascade-mode units (timers 1-3 only, when their own CNT_H cascade bit is set) run no `ptimer` at all -- `counter` is incremented directly by the previous unit's overflow handler and checked for wraparound there.
- **Phase 5 — DMA.** `hw/dma/gba_dma.c`, 4 channels, immediate/VBlank/HBlank/special start modes. DMA timing (cycle-stealing) deferred to a later accuracy pass — tracked explicitly in `PROGRESS.md`, not silently dropped. **Done.** Each channel's registers span a contiguous 12 bytes (`SAD`/`DAD`/`CNT_L`/`CNT_H`), all 4 channels mapped as one 48-byte MMIO block at `0x040000B0`. A transfer runs as a synchronous burst of `address_space_read`/`address_space_write` calls at trigger time (immediate mode fires on the write that sets the enable bit; VBlank/HBlank/special fire from `qdev_init_gpio_in_named` lines -- `"vblank"`/`"hblank"`/`"special"` (2 lines, hardwired to DMA1/DMA2 for the sound FIFO-empty signal) -- left unconnected until the PPU (Phase 6) and APU (Phase 7) exist to drive them). Repeat-mode channels keep live `cur_src`/`cur_dst` state across firings so non-reload address-control modes (increment/decrement/fixed) continue rather than restart; only dest-control "increment/reload" (mode 3) resets on each repeat.
